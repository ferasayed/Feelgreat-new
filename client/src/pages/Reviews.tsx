...
export default function Reviews() {
  ...
  useEffect(() => {
    document.title = "Customer Reviews | Feel Great Success Stories";
  }, []);
      ...
      <section className="py-16 text-center">
        <div className="container max-w-3xl mx-auto">
          ...
          <p className="text-lg text-muted-foreground mb-6">
            Real experiences from real people on their health transformation journey
          </p>
          {stats && (
            <div className="flex items-center justify-center gap-8 mb-8">
              ...
              <div className="text-center">
                ...
                <div className="text-xs text-muted-foreground mt-1">Published Reviews</div>
              </div>
            </div>
          )}
        ...
        <section className="container max-w-lg mx-auto pb-12">
          <Card className="border-0 shadow-xl">
            <CardContent className="p-8">
              <h2 className="text-xl font-bold mb-6">Share Your Review</h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Your Name *</label>
                  <Input value={formData.name} onChange={(e) => setFormData((p) => ({ ...p, name: e.target.value }))} placeholder="e.g. Sarah M." required />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Country *</label>
                  <Input value={formData.country} onChange={(e) => setFormData((p) => ({ ...p, country: e.target.value }))} placeholder="e.g. United States" required />
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input value={formData.title} onChange={(e) => setFormData((p) => ({ ...p, title: e.target.value }))} placeholder="e.g. Life-changing results in 3 months" required />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1">Your Experience *</label>
                  <Textarea value={formData.content} onChange={(e) => setFormData((p) => ({ ...p, content: e.target.value }))} placeholder="Tell us about your journey with Feel Great..." rows={4} required />
                </div>
