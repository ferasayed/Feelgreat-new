...
const SHARE_UI: Record<string, { share: string; copyLink: string; copied: string; shareVia: string }> = {
  ar: { share: "شارك", copyLink: "نسخ الرابط", copied: "تم نسخ الرابط!", shareVia: "شارك عبر" },
  en: { share: "Share", copyLink: "Copy Link", copied: "Link copied!", shareVia: "Share via" },
  fr: { share: "Partager", copyLink: "Copier le lien", copied: "Lien copié !", shareVia: "Partager via" },
  es: { share: "Compartir", copyLink: "Copiar enlace", copied: "¡Enlace copiado!", shareVia: "Compartir vía" },
  de: { share: "Teilen", copyLink: "Link kopieren", copied: "Link kopiert!", shareVia: "Teilen über" },
  tr: { share: "Paylaş", copyLink: "Bağlantıyı kopyala", copied: "Bağlantı kopyalandı!", shareVia: "Şununla paylaş" },
};

export default function ShareButtons({ url, title, description, lang = "en", variant = "horizontal" }: ShareButtonsProps) {
  ...
  if (variant === "floating") {
    return (
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 bg-background/95 backdrop-blur-md border border-border shadow-xl rounded-full px-4 py-2.5 transition-all duration-300">
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className="text-sm text-muted-foreground font-medium">{t.share}:</span>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-[#0088cc]/10 text-[#0088cc] hover:bg-[#0088cc]/20 transition-colors"
        title="Telegram"
      >
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
          <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.479.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-foreground/5 text-foreground hover:bg-foreground/10 transition-colors"
        title="X"
      >
        <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="currentColor">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
